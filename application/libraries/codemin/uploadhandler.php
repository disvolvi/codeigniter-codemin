...
class UploadHandler
{
    ...
    function __construct($diretorio = null, $options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/uploads/'.$diretorio['tipo'].'/'.$diretorio['tabela'].'/'.$diretorio['registro'].'/'.$diretorio['campo'].'/',
            'upload_url' => $this->get_full_url().'/uploads/'.$diretorio['tipo'].'/'.$diretorio['tabela'].'/'.$diretorio['registro'].'/'.$diretorio['campo'].'/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            'tipo' => $diretorio['tipo'],
            'tabela' => $diretorio['tabela'],
            'registro' => $diretorio['registro'],
            'campo' => $diretorio['campo'],
            'file' => $diretorio['file'],
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url']
            .'uploader/index/'.$this->options['tipo']
            .'/'.$this->options['tabela']
            .'/'.$this->options['registro']
            .'/'.$this->options['campo']
    ...
    protected function get_file_name_param() {
        return isset($this->options['file']) ? basename(stripslashes($this->options['file'])) : null;
    }
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                ...
                $inserir = array(
                    ...
                    'tabela' => $this->options['tabela'],
                    'registro' => $this->options['registro'],
                    'campo' => $this->options['campo'],
                    'arquivo' => $file->name
                );
                $this->db->insert('codemin_uploads',$inserir);
                $files[] = $file;
            }
        } else {
            ...
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
                    ...
                    if (is_file($file)) {
                        ...
                        $this->db->where($where);
                        $this->db->delete('codemin_uploads');
                    }
                }
            }
        }
